...
import { formatDateTime, formatRelativeTime, t } from '@/lib/locale';
    ...
    return (
        <GreyRowBox css={tw`flex-wrap md:flex-nowrap items-center`} className={className}>
            ...
            <div css={tw`flex-1 md:flex-none md:w-48 mt-4 md:mt-0 md:ml-8 md:text-center`}>
                <p title={formatDateTime(backup.createdAt, 'ddd, MMMM do, yyyy HH:mm:ss', 'yyyy/MM/dd HH:mm:ss')} css={tw`text-sm`}>
                    {formatRelativeTime(backup.createdAt, { includeSeconds: true, addSuffix: true })}
                </p>
                <p css={tw`text-2xs text-neutral-500 uppercase mt-1`}>{t('ui.server.backups.created')}</p>
            </div>
