...
const ActivePill = ({ active }: { active: boolean }) => (
    ...
    >
        {active ? t('ui.server.statuses.active') : t('ui.server.statuses.inactive')}
    </span>
);
...
export default () => {
    ...
    return (
        <PageContentBlock title={t('ui.server.schedules.title')}>
                ...
                <>
                    ...
                    <div css={tw`rounded shadow`}>
                            ...
                            <div css={tw`flex-1`}>
                                ...
                                <p css={tw`mt-1 text-sm text-neutral-200`}>
                                    ...
                                    {schedule.lastRunAt ? (
                                        ...
                                        <span css={tw`text-neutral-300`}>{t('ui.common.not_available')}</span>
                                    )}
                                    <span css={tw`ml-4 pl-4 border-l-4 border-neutral-600 py-px`}>
                                        {t('ui.server.schedules.next_run_at')}&nbsp;
                                        {schedule.nextRunAt ? (
                                            formatDateTime(schedule.nextRunAt, "MMM do 'at' h:mma", 'yyyy/MM/dd HH:mm')
                                        ) : (
                                            <span css={tw`text-neutral-300`}>{t('ui.common.not_available')}</span>
                                        )}
                                    </span>
                        ...
                        <div css={tw`hidden sm:grid grid-cols-5 md:grid-cols-5 gap-4 mb-4 mt-4`}>
                            <CronBox title={t('ui.server.schedules.minute')} value={schedule.cron.minute} />
                            <CronBox title={t('ui.server.schedules.hour')} value={schedule.cron.hour} />
                            <CronBox title={t('ui.server.schedules.cron_day_of_month')} value={schedule.cron.dayOfMonth} />
                            <CronBox title={t('ui.server.schedules.month')} value={schedule.cron.month} />
                            <CronBox title={t('ui.server.schedules.cron_day_of_week')} value={schedule.cron.dayOfWeek} />
                        </div>
